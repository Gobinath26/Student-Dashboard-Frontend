...
function Tasknotgraded({user}) {
  ...
  useEffect(async () => {
    ...
    let fetchtask = async () => {
      try {
         let itemdetials = await axios.get(`https://dashboard-backendd.herokuapp.com/notgraded/${user}`,{
          headers: {
    ...
    let handleDelete = async (id) => {
        try {
            ...
            if (result) {
                await axios.delete(`https://dashboard-backendd.herokuapp.com/deletetask/${id}` )
                fetchtask()
            }
        } catch (error) {
            console.log(error)
