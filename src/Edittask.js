...
function Edittask({user}) {
    ...
    useEffect(async () => {
        let userData = await axios.get(`https://dashboard-backendd.herokuapp.com/getbyid/${params.id}`);
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
        try {
            console.log(values);
           var response = await axios.put(`https://dashboard-backendd.herokuapp.com/edittask/${params.id}`, values);
            alert("Task Edited");
            navigate("/dashboard");
