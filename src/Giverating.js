...
function Giverating() {
  ...
  let fetchtask = async () => {
    try {
      let itemdetials = await axios.get(`https://dashboard-backendd.herokuapp.com/gradetask/${params.id}`, {
        headers: {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      try {
        console.log(values);
        let data = await axios.put(`https://dashboard-backendd.herokuapp.com/gradetask/${params.id}`, values)
        navigate('/rate')
      } catch (error) {
        console.log(error)
      }
